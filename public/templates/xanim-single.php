<?php
/**
 * Bibiheybet.com - Xanım haqqında Tək Məqalə
 */
...
$stmt = $db->prepare("SELECT * FROM xanim_haqqinda WHERE {$slugField} = :slug AND status = 'published' LIMIT 1");
$stmt->execute([':slug' => $pageSlug]);
$xanim = $stmt->fetch();

if (!$xanim && $lang !== 'az') {
    $stmt = $db->prepare("SELECT * FROM xanim_haqqinda WHERE slug_az = :slug AND status = 'published' LIMIT 1");
    $stmt->execute([':slug' => $pageSlug]);
    $xanim = $stmt->fetch();
}
...
$galleryStmt = $db->prepare(
    "SELECT * FROM xanim_gallery WHERE xanim_id = :xid ORDER BY sort_order ASC"
);
$galleryStmt->execute([':xid' => $xanim['id']]);
$gallery = $galleryStmt->fetchAll();

$GLOBALS['bb_page_context']['alternate_urls'] = bb_get_alternate_urls('xanim', [
    'az' => $xanim['slug_az'] ?? '',
    'en' => $xanim['slug_en'] ?? '',
    'ru' => $xanim['slug_ru'] ?? '',
    'ar' => $xanim['slug_ar'] ?? '',
...
$seoData = bb_prepare_seo_data($xanim, 'xanim', $lang);
$seoData['canonical_url'] = bb_lang_url(
    bb_get_route('xanim', $lang) . '/' . bb_get_field($xanim, 'slug', $lang),
    $lang
);
$seoData['alternate_urls'] = $GLOBALS['bb_page_context']['alternate_urls'];

$xName    = bb_get_field($xanim, 'name', $lang);
$xContent = bb_get_field($xanim, 'content', $lang);
$xImage   = bb_get_featured_image($xanim, $lang);
...
$shareTitle = urlencode($xName ?? '');

bb_frontend_header([
    'seo_data'   => $seoData,
    'body_class' => 'bb-page-xanim-single',
...
?>
    ...
    <div class="bb-container">
        <?= bb_render_breadcrumbs([
            ...
            ['label' => $t('back'), 'url' => bb_lang_url(bb_get_route('xanim-haqqinda', $lang) . '/', $lang)],
            ['label' => $xName],
        ]) ?>
    </div>
    ...
    <section class="bb-ps-content-section">
        <div class="bb-container-narrow">
            <h1 class="bb-ps-title"><?= bb_sanitize($xName) ?></h1>
            <div class="bb-separator"></div>

            <?php if ($xImage): ?>
            <div class="bb-ps-featured">
                <img src="/<?= bb_sanitize($xImage) ?>"
