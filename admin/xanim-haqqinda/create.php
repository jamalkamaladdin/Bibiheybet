<?php
/**
 * Bibiheybet.com - Yeni Xanım haqqında Məqalə Yarat
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!bb_verify_csrf($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($errors)) {
            $stmt = $db->prepare("SELECT COUNT(*) FROM xanim_haqqinda WHERE slug_az = :s");
        ...
        if (empty($errors)) {
            ...
            bb_redirect('/admin/xanim-haqqinda/edit.php?id=' . $newId);
        }
    }
}
