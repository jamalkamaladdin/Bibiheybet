<?php
/**
 * Bibiheybet.com - Xanım haqqında Qalereya Başlıq Yeniləmə (AJAX)
 */
...
$allowedLangs = function_exists('bb_all_langs') ? bb_all_langs() : ['az', 'en', 'ru', 'ar', 'fa'];
...
$stmt = $db->prepare("SELECT id FROM xanim_gallery WHERE id = :id LIMIT 1");
...
$stmt = $db->prepare("UPDATE xanim_gallery SET {$column} = :caption WHERE id = :id");
$stmt->execute([
